...
# Cargar el DataFrame
try:
    df = pd.read_excel('./db_mia.xlsx')
    df.drop(['gbifID', 'datasetKey', 'publishingOrgKey', 'eventDate', 'typeStatus', 'Ver_Detalles'], axis=1, inplace=True)
    df['total_text'] = df.apply(lambda x: f"{x['verbatimScientificName']} {x['description']} {x['kingdom']} {x['phylum']} {x['class']} {x['species']} {x['stateProvince']}", axis=1)
