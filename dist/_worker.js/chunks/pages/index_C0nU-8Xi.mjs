...
import { $ as $$Layout } from './animalae_xQXar2uY.mjs';
/* empty css                          */
...
const $$ImportantValue = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section class="flex flex-col w-full md:w-48 items-center"> <img class="w-20 h-20"${addAttribute(icon, "src")}${addAttribute(description, "alt")}> <h2>${title}</h2> <a class="text-center font-bold text-sm">${description}</a> </section>`;
...
const $$DataGeneral = createComponent(($$result, $$props, $$slots) => {
  ...
  const { title, description, img1, img2 } = Astro2.props;
  return renderTemplate`${maybeRenderHead()}<section class="mt-1"> <div${addAttribute(`gap-16 items-center py-8 px-4 mx-auto max-w-screen-xl lg:grid  lg:py-16 lg:px-6 ${img1 && img2 ? "lg:grid-cols-2" : ""}`, "class")}> <div class="font-light text-white sm:text-lg"> <h2 class="mb-4 text-4xl tracking-tight font-extrabold text-green-300"> ${title} </h2> <p class="mb-4"> ${description} </p> </div> ${img1 && img2 && renderTemplate`<div class="grid grid-cols-2 gap-4 mt-8"> <img class="object-cover rounded-lg w-48 h-56"${addAttribute(
    img1 || "https://flowbite.s3.amazonaws.com/blocks/marketing-ui/content/office-long-2.png",
    "src"
  )} alt="office content 1"> <img class="object-cover rounded-lg w-48 h-80"${addAttribute(
    img2 || "https://flowbite.s3.amazonaws.com/blocks/marketing-ui/content/office-long-1.png",
    "src"
  )} alt="office content 2"> </div>`} </div> </section>`;
